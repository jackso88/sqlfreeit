...
SELECT course, AVG(DATEDIFF(year, br_date, GETDATE())) AS 'средний возраст' FROM teachers
GROUP BY course
...
SELECT course, COUNT(role_in_course) AS 'count' FROM teachers
WHERE role_in_course LIKE 'helper'
GROUP BY course;
...
SELECT ROUND(CAST(CAST(SUM(lect_hours) AS DECIMAL)/CAST(SUM(labs_hours) AS DECIMAL) AS DECIMAL (6,3)),3) AS 'отношение' FROM teachers
...
SELECT course, ROUND(CAST(CAST(SUM(labs_hours) AS DECIMAL)/CAST(SUM(lect_hours) AS DECIMAL) AS DECIMAL (6,2)),2) AS 'отношение' FROM teachers
GROUP BY course
HAVING ROUND(CAST(CAST(SUM(labs_hours) AS DECIMAL)/CAST(SUM(lect_hours) AS DECIMAL) AS DECIMAL (6,2)),2) >= 2 AND SUM(lect_hours) != 0;

--- 14. Найти курсы, на которых число студентов с кодом 29 и восьмеркой в номере больше двух. 
SELECT course, COUNT(phone) AS 'count' FROM studs

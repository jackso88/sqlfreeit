...
--- Создание связей
ALTER TABLE form ADD CONSTRAINT FK_FORM FOREIGN KEY (id_faculty) REFERENCES faculty (id);
ALTER TABLE courses ADD CONSTRAINT FK_COURS_FORM FOREIGN KEY (id_form) REFERENCES form (id);
...
INSERT INTO form
SELECT DISTINCT form, faculty.id FROM teach
...
RIGHT JOIN (SELECT DISTINCT form.id AS ID, form.form_name AS form, faculty.faculty_name AS faculty, teach.year AS year FROM faculty
			JOIN form ON form.id_faculty = faculty.id
...
JOIN (SELECT courses.id AS id, courses.year AS year, form.form_name AS form, faculty.faculty_name AS faculty FROM courses
		JOIN form ON form.id = courses.id_form
		JOIN faculty ON faculty.id = form.ID_faculty) tmp ON teach.year = tmp.year AND teach.form = tmp.form AND teach.faculty = tmp.faculty
...
JOIN (SELECT courses.id AS id, courses.year AS year, form.form_name AS form, faculty.faculty_name AS faculty FROM courses
		JOIN form ON form.id = courses.id_form
		JOIN faculty ON faculty.id = form.ID_faculty) tmp ON stud.year = tmp.year AND stud.form = tmp.form AND stud.faculty = tmp.faculty
